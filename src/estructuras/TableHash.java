...
public class TableHash {
    ...
    public void insertar(String nombreVariable,String Tipo, String Ambito, String Valor ){
        boolean insertado=false;
        if(porcentajeUtil<=65.00f){
            for (int i = 0; i < tamaño; i++) {
                int posicion= DobleHashing(nombreVariable, i);
                if(posicion>tamaño){
                    posicion-=tamaño;
                }
                if(vectorHash[posicion]==null || vectorHash[posicion].estado=='b'){
                    vectorHash[posicion] = new nodoHash(nombreVariable, Tipo, Ambito,Valor  );
                    ocupados+=1;
                    porcentajeUtil=calcularPorcentajeUtil();
                    insertado=true;
                    break;
                }else{
                    if(vectorHash[posicion].NombreVariable.equals(nombreVariable)){
                        System.out.println("La variable a insertar ya existe en latabla: "+nombreVariable);
                        break;
                    }else{
            ...
            rehashing();
            insertar(nombreVariable, Tipo, Ambito, Valor);
        }
    }
    ...
    private void rehashing() {
        ...
        for (int i = 0; i < tamañotmp; i++) {
            if(temp[i]!=null){
                insertar(temp[i].NombreVariable, temp[i].tipo, temp[i].ambito, temp[i].valor);
            }
    ...
    public nodoHash ExtraerNodo(String nombreVariable){
        ...
        if(encontrado){
            ...
            System.out.println("Lavariable "+nombreVariable+"nose ecuentra en la tabla");
        }
        return tmp;
    }
}
